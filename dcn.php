...
// QUERY PARA MOSTRAR SOMENTE OS REGISTROS A PARTIR DA DATA DE HOJE
$strSQL = "SELECT * FROM $table WHERE setor LIKE '%Dcn%' AND DATE(dt_plantao) >= CURDATE() ORDER BY dt_plantao, hora_ent, funcao, nome";
...
echo "<table class='table table-hover table-bordered espace'>
		 <caption><h3>Escala DCN (A partir de Hoje)</h3></caption>
		 <thead>
			<tr>
				<th>Nome</th>
				<th>Celular</th>
...
?> 
	...
	function CountDown(){
	    ...
	    if(counter > 0){
			...
			window.location = "dcn.php"; //URL DA PAGINA A SER REDIRECIONADA
		}
	}
